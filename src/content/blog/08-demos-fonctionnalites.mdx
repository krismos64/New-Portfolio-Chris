...
SmartPlanning adapte son interface au rôle de l'utilisateur connecté. Chacune des trois démos ci-dessous parcourt en immersion l'espace dédié à un rôle. On part du **directeur** qui pilote l'organisation, on descend au **manager** qui supervise son équipe, puis on termine côté **employé** qui consulte ses propres informations.

### Espace Directeur : Pilotage global

Vue d'ensemble de la plateforme : indicateurs clés (effectifs totaux, équipes, congés en attente), graphiques Recharts (répartition par équipe en donut, évolution des effectifs), accès complet à la gestion d'équipes, aux abonnements Stripe et à l'export des données.

<VideoDemo
  src="K4bidIhB-LI"
  title="SmartPlanning : Démo de l'espace Directeur"
  type="youtube"
/>

### Espace Manager : Supervision d'équipe

Le manager gère uniquement son périmètre : création et modification des plannings de son équipe, validation des demandes de congés, suivi des heures travaillées et messagerie avec ses collaborateurs. Les permissions sont strictement cloisonnées pour respecter la confidentialité des autres équipes.

<VideoDemo
  src="sy3ffTIj0ig"
  title="SmartPlanning : Démo de l'espace Manager"
  type="youtube"
/>

### Espace Employé : Vue personnelle

L'employé accède à son planning de la semaine, ses congés à venir, ses tâches personnelles et la messagerie. Interface volontairement épurée : pas de surcharge informationnelle, juste les actions utiles au quotidien (déposer un congé, consulter ses créneaux, échanger avec son manager).

<VideoDemo
  src="8wJgk8i1uzQ"
  title="SmartPlanning : Démo de l'espace Employé"
  type="youtube"
/>

**Points clés des trois démos :**

- 3 espaces distincts avec permissions strictement cloisonnées (Directeur, Manager, Employé)
...
- Dashboard Manager : périmètre limité à son équipe, validation des congés, supervision des plannings
- Dashboard Employé : planning de la semaine en cours, congés à venir, tâches personnelles
- Démonstration concrète de la granularité du contrôle d'accès (RBAC) en production
